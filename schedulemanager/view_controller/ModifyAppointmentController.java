...
public class ModifyAppointmentController implements Initializable {
    ...
    public void setAppointment(Appointment appointment) {
        ...
        this.customer = mainApp.getCustomerManager().getCustomer(appointment.getCustomerID());
        ...
        apptCustomerTable.getSelectionModel().select(customer);
        
        startTimeList.setItems(ClockLister.getClock());
        LocalTime startTime = appointment.getStart().toLocalDateTime().toLocalTime();
        int indexOfStartTime = ClockLister.indexOfTime(startTime);
        startTimeList.getSelectionModel().select(indexOfStartTime);
        startTimeList.scrollTo(indexOfStartTime);
        
        endTimeList.setItems(ClockLister.getClock());
        LocalTime endTime = appointment.getEnd().toLocalDateTime().toLocalTime();
        int indexOfEndTime = ClockLister.indexOfTime(endTime);
        endTimeList.getSelectionModel().select(indexOfEndTime);
        endTimeList.scrollTo(indexOfEndTime);
        
        apptDatePicker.setValue(appointment.getStart().toLocalDateTime().toLocalDate());
    ...
    public void handleSave() throws SQLException {
        if(validInput()) {
            appointment.setCustomerID(apptCustomerTable.getSelectionModel().getSelectedItem().getCustomerID());
            ...
            mainApp.getAppointmentManager().updateAppointment(mainApp.getConnection(), appointment,
                                            mainApp.getLoggedInUser().getUserName());
            saveClicked = true;
            dialogStage.close();
        }
        else {
